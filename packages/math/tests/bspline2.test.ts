...
describe('BSpline2', () => {
    ...
    it('constructs from expanded knots and evaluates curve', () => {
        const c = new BSpline2(cps, degree, { expandedKnots: knots })
    ...
    it('supports compact knot input and mismatch checks', () => {
        const c = new BSpline2(cps, degree, {
            knots: [0, 1],
            multiplicities: [3, 3],
        })
        expect(c.pointAt(0.5).equals(new Vec2(1, 0), 1e-9)).toBe(true)

        expect(() => new BSpline2(cps, degree, {
            expandedKnots: knots,
            knots: [0, 1],
            multiplicities: [2, 4],
        })).toThrow('BSpline2: expandedKnots mismatch with knots+multiplicities')
    })

    it('enforces periodic and weight constraints', () => {
        expect(() => new BSpline2(cps, degree, { expandedKnots: knots, isPeriodic: true })).toThrow('BSpline2: periodic is not supported in v1')
        expect(() => new BSpline2(cps, degree, { expandedKnots: knots, weights: [1, -1, 1] })).toThrow('BSpline2: weight must be > 0')
        expect(() => new BSpline2(cps, 0, { expandedKnots: knots })).toThrow('BSpline2: degree must be an integer >= 1')
        expect(() => new BSpline2([new Vec2(0, 0), new Vec2(1, 1)], degree, { expandedKnots: knots })).toThrow('BSpline2: controlPoints.length must be >= degree + 1')
        expect(() => new BSpline2(cps, degree, { weights: [1, 1, 1] })).toThrow('BSpline2: expandedKnots or (knots+multiplicities) is required')
        expect(() => new BSpline2(cps, degree, { knots: [0, 1], multiplicities: [3] })).toThrow('BSpline2: knots and multiplicities length mismatch')
        expect(() => new BSpline2(cps, degree, { knots: [0, 1], multiplicities: [2, 4] })).toThrow('BSpline2: invalid parameter domain')
        expect(() => new BSpline2(cps, degree, { expandedKnots: [0, 0, 0, 1, Number.NaN, 1] })).toThrow('BSpline2: knot must be finite')
        expect(() => new BSpline2(cps, degree, { expandedKnots: [0, 0, 0, 1, 0.5, 1] })).toThrow('BSpline2: knots must be non-decreasing')
        expect(() => new BSpline2(cps, degree, { knots: [0, 1], multiplicities: [3, 0] })).toThrow('BSpline2: multiplicity must be positive integer')
    })
    ...
    it('length/paramAtLength/split/trim', () => {
        ...
        const tiny = new BSpline2(
            [new Vec2(0, 0), new Vec2(5e-13, 0), new Vec2(1e-12, 0)],
            2,
    ...
    it('closestPoint, reverse, transform and dump/load', () => {
        ...
        const changed = new BSpline2(cps, degree, { expandedKnots: knots, weights: [1, 2, 1] })
        expect(c.equals(changed)).toBe(false)

        const changedDegree = new BSpline2([new Vec2(0, 0), new Vec2(1, 0), new Vec2(2, 0), new Vec2(3, 0)], 3, {
            expandedKnots: [0, 0, 0, 0, 1, 1, 1, 1],
        })
        expect(c.equals(changedDegree as unknown as BSpline2)).toBe(false)
        const changedCpCount = new BSpline2([new Vec2(0, 0), new Vec2(1, 0), new Vec2(2, 0), new Vec2(3, 0)], 2, {
            expandedKnots: [0, 0, 0, 1, 2, 2, 2],
        })
        expect(c.equals(changedCpCount as unknown as BSpline2)).toBe(false)
        const changedWeightCount = c.clone()
        ;(changedWeightCount as unknown as { _weights: number[] })._weights = [1, 1]
        expect(c.equals(changedWeightCount)).toBe(false)
        const changedKnotCount = c.clone()
        ;(changedKnotCount as unknown as { _knots: number[] })._knots = [0, 0, 1]
    ...
    it('covers remaining algorithm branches and validity guards', () => {
        ...
        const invalidWeights = nonlinear as unknown as { _weights: number[] }
        invalidWeights._weights = [1]
        expect(nonlinear.isValid()).toBe(false)

        invalidWeights._weights = Array<number>(6).fill(1)
        const invalidKnots = nonlinear as unknown as { _knots: number[] }
        ...
        ;(other as unknown as { _weights: number[] })._weights = [...(other as unknown as { _weights: number[] })._weights]
        ;(other as unknown as { _weights: number[] })._weights[0] += 0.1
        expect(nonlinear.equals(other)).toBe(false)
        ;(other as unknown as { _weights: number[] })._weights[0] = 1
        ;(other as unknown as { _knots: number[] })._knots = [...(other as unknown as { _knots: number[] })._knots]
        ;(other as unknown as { _knots: number[] })._knots[0] += 0.1
        expect(nonlinear.equals(other)).toBe(false)

        const privateApi = BSpline2 as unknown as {
            insertKnotOnce: (points: { x: number; y: number; w: number }[], knots: number[], degree: number, u: number) => unknown
            basisFunctionDerivatives: (span: number, u: number, p: number, n: number, U: readonly number[]) => number[][]
        }

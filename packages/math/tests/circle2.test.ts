...
describe('Circle2', () => {
    it('uses periodic range and analytic evaluation', () => {
        ...
        expect(c.getPtAt(0).equals(new Vec2(4, 2), 1e-12)).toBe(true)
        ...
        expect(c.getPtAt(Math.PI * 4 + Math.PI / 3).equals(c.getPtAt(Math.PI / 3), 1e-9)).toBe(true)
    })
    ...
    it('dump/load round-trip', () => {
        ...
        expect(restored.getPtAt(Math.PI / 3).equals(c.getPtAt(Math.PI / 3), 1e-9)).toBe(true)
